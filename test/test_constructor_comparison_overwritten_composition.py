...
def test_constructor():
    ...
    def test_vec_scalar():
        z = 2*x+y**2
        assert z.get_value() == 3.0
        assert type(z.get_value()) == float
        assert z._get_derivative_of(x) == 2.0
        assert z._get_derivative_of(y) == 4.0
        assert type(z._get_derivative_of(x)) == 2.0
        assert type(z._get_derivative_of(y)) == 4.0
    ...
    def test_vector_vector():
        ...
        assert z.get_der([x,y]) == [[1.0, 0], [1.0, 4.0]]
        ## partial derivative
...
def test_vector_scalar():
    def test_overloading():
        ...
        def test_div():
            z1 = (z/x)/10
            assert z2.get_value() == 5.0/40.0
            assert z2.get_der() == [1.0/40.0, -5.0/160.0]
        ...
        def test_pow():
            ...
            assert np.round(z1.get_der(),4) == np.round([48.0, 64*np.log(4)], 4)
            assert np.round(z1._get_derivative_of(x), 4) == np.round(48.0, 4)
            assert np.round(z1._get_derivative_of(y), 4) == np.round(64*np.log(4), 4)

        def test_rpow():
            z1 = (2**x)**y
            assert z1.get_value() == 4096.0
            assert np.round(z1.get_der(), 4) == np.round([12288*np.log(2), 16384*np.log(2)], 4)
            assert np.round(z1._get_derivative_of(x), 4) == np.round(12288*np.log(2), 4)
            assert np.round(z1._get_derivative_of(y), 4) == np.round(16384*np.log(2), 4)
    ...
    def test_composition():
        ...
        def test_1_trig():
            ...
            assert np.round(z1.get_value(),10) == -0.8414709848
            assert np.round(z1.get_der(),10) == [0, 1.0]
            assert np.round(z1._get_derivative_of(x), 4) == np.round(0, 4)
            assert np.round(z1._get_derivative_of(y), 4) == np.round(1.0, 4)
...
def test_scalar_vector():
    def test_overloading():
        ...
        def test_rsubtract():
            z1 = 10.0-y-z
            assert z1.get_value() == [-38.0, -14.0]
            assert z1.get_der([x]) == [-14.0, 0]

        def test_mul():
            z1 = y*z*2
            assert z1.get_value() == [64.0, 256.0]
            assert z1.get_der([x]) == [32.0, 96.0]
        ...
        def test_div():
            ...
            assert z1.get_der([x]) == [0, 0.05]

        def test_rdiv():
            z1 = 10/(y/x)
            assert z1.get_value() == [5.0, 5.0]
            assert z1.get_der([x]) == [0, -1.25]
        ...
        def test_rpow():
            ...
            assert np.round(z1.get_der([x]), 4) == np.round([512.0*np.log(2), 524288.0*np.log(2)], 4)
    ...
    def test_composition():
        ...
        def test_1_trig():
            ...
            z1 = f.apply(Var.sin)
            assert np.round(z1.get_value(),10) == -0.8414709848
            assert np.round(z1.get_der([x]),10) == [0, 0]
...
def test_vector_vector():
    def test_overloading():
        ...
        def test_mul():
            ...
            assert z2.get_der([x,y]) == [[4.0, 12.0], [2.0, 5.0]]
            assert z2.get_der([x]) == [[4.0], [2.0]]
            assert z2.get_der([y]) == [[12.0], [5.0]]
        ...
        def test_div():
            ...
            assert z1.get_der() == [[0.1, 0], [0, 0.1]]
            assert z1.get_der([x]) == [[0.1], [0]]
            assert z1.get_der([y]) == [[0], [0.1]]
            
            z2 = p/z
            assert np.round(z2.get_value(), 4) == np.round([4.0/3, 2.5], 4)
            assert np.round(z2.get_der([x,y]), 10) == np.round([[-4.0/9, 2.0/3], [0.5, -5.0/4]], 10)
            assert np.round(z2.get_der([x]),10) == np.round([[-4.0/9], [0.5]], 10)
            assert np.round(z2.get_der([y]), 10) == np.round([[2.0/3], [-5.0/4]], 10)

        def test_rdiv():
            z1 = 10/z
            assert np.round(z1.get_value(), 10) == np.round([10.0/3, 5.0], 10)
            assert np.round(z1.get_der([x,y]), 4) == np.round([[-10.0/9, 0], [0, 2.5]], 10)
            assert np.round(z1.get_der([x]),10) == np.round([[-10.0/9], [0]], 10)
            assert np.round(z1.get_der([y]), 10) == np.round([[0], [2.5]], 10)
        ...
        def test_pow():
            ...
            assert np.round(z2.get_der([x,y]), 10) == np.round([[108.0, 162.0*np.log(3)], [32.0*np.log(2), 80.0]], 10)
            assert np.round(z2.get_der([x]),10) == np.round([[108.0], [32.0*np.log(2)]], 10)
            assert np.round(z2.get_der([y]), 10) == np.round([[162.0*np.log(3)], [80.0]], 10)

        def test_rpow():
            z1 = 2**z
            assert z1.get_value() == [8.0, 4.0]
            assert np.round(z1.get_der([x,y]), 10) == np.round([[8.0*np.log(2), 0], [0, 4.0*np.log(2)]], 10)
            assert np.round(z1.get_der([x]),10) == np.round([[8.0*np.log(2)], [0]], 10)
            assert np.round(z1.get_der([y]), 10) == np.round([[0], [4.0*np.log(2)]], 10)
    ...
    def test_composition():
        ...
        def test_1_trig():
            ...
            y = Var(np.pi/2)
            z = MultiFunc([x, y])
            z1 = Var.sin(Var.cos(z))
            assert np.round(z1.get_value(),10) == [-0.8414709848, 0]
            assert z1.get_der([x,y]) == [[0, 0], [0, -1.0]]  # = -sin(x)cos(cos(x)) - sin(y)cos(cos(y))
            assert z1.get_der([x]) == [[0], [0]]
            assert z1.get_der([y]) == [[0], [-1.0]]
        ...
        def test_2():
            ...
            assert z1.get_der([x,y]) == [[4.0,0], [0, 6.0]]
            assert z1.get_der([x]) == [[4.0], [0]]
            assert z1.get_der([y]) == [[0], [6.0]]
